...
def load_embedding_matrix(pickle_file,
    ...
    if document_id_type not in ['scirex', 's2orc']:
        raise ValueError(
            "Only 2 possible values supported for document_id_type: scirex or s2orc")
    ...
    for doc_id in tqdm(document_ids):
        ...
        if converted_doc_id != None and converted_doc_id in s2orc_id_to_node_id_mapping:
    ...
    matrix, average_embedding = construct_embedding_matrix(
        full_graph_embeddings, doc_id_to_full_matrix_idx_mapping, doc_idx_mapping)
    return matrix, doc_idx_mapping, average_embedding


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--graph_embeddings_pickle', type=str,
                        required=True, help="GraphVite embedding pickle file")
    parser.add_argument('--node_to_id_mapping', type=str, required=True,
                        help="Node-name-to-node-index JSON file, produced before learning graph embedding")
    parser.add_argument('--out_embedding_file', type=str, required=True,
                        help="Numpy binary file containing document embedding matrix")
    parser.add_argument('--out_doc_to_index_mapping_file', type=str,
                        required=True, help="JSON file containing doc-id-to-embedding-index")
    args = parser.parse_args()
    all_scirex_documents = list(get_scirex_docids())
    matrix, scirex_doc_idx_mapping, _ = load_embedding_matrix(
        args.graph_embeddings_pickle, args.node_to_id_mapping, all_scirex_documents, document_id_type='scirex')

    np.save(args.out_embedding_file, matrix)
    write_scirex_index_mapping_to_file(
        scirex_doc_idx_mapping, args.out_doc_to_index_mapping_file)
