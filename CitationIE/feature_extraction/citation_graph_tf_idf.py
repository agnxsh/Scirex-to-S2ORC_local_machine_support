...
def tokenize_text(full_text_dict):
    all_text = " ".join(section["text"]
                        for section in full_text_dict["body_text"])
...
def construct_dictionary(document_reader):
    ...
    dictionary = corpora.Dictionary(tokenize_text(line)
                                    for line in document_reader)
    ...
    print(
        f"Constructing dictionary took {function_end - function_start} seconds")
    return dictionary
...
class JsonlinesCorpus:
    ...
    def __iter__(self):
        for i, doc in enumerate(jsonlines.open(self._fname)):
            if self._num_documents_to_load != None and i >= self._num_documents_to_load:
                break
...
def compute_tf_idf_matrix(documents_path, dictionary, num_documents_to_load=None, memory_mapping_file=MEMORY_MAPPING_FILE):
    ...
    if not os.path.exists(memory_mapping_file):
        ...
        corpus = JsonlinesCorpus(
            documents_path, dictionary, num_documents_to_load=num_documents_to_load)
        corpora.MmCorpus.serialize(
            MEMORY_MAPPING_FILE, corpus, progress_cnt=1000, metadata=True)
        print(
            f"Generated memory mapping for corpus. Took {time.perf_counter() - mm_start} seconds.")
    else:
        print(
            f"Memory mapping file already exists at {MEMORY_MAPPING_FILE}, just gonna load that.")
    ...
    print(
        f"Building TF-IDF model took {time.perf_counter() - build_tfidf_start} seconds.")

    corpus = JsonlinesCorpus(documents_path, dictionary,
                             num_documents_to_load=num_documents_to_load)
    ...
    tf_idf_matrix = sparse.csr_matrix(
        (data, (rows, cols)), shape=(num_docs, len(dictionary)))
    function_end = time.perf_counter()
    print(
        f"Computing tf-idf matrix took {function_end - function_start} seconds")
...
def compute_average_term_tfidfs_for_documents(adjacency_list, tf_idf_matrix, term_dictionary, doc_idx_mapping):
    ...
    function_end = time.perf_counter()
    print(
        f"Averaging TF-IDFs over neighbors took {function_end - function_start} seconds")
...
def main():
    ...
    parser.add_argument('--out-directory',
                        default="s2orc_caches/fulltexts/tf_idfs/")
    ...
    if args.clear_cache:
        if os.path.exists(TERM_DICTIONARY_FILE):
            ...
            print(
                f"Removed term dictionary file {TERM_DICTIONARY_FILE} successfully")
        if os.path.exists(MEMORY_MAPPING_FILE):
            os.remove(MEMORY_MAPPING_FILE)
            print(
                f"Removed corpus memory mapping file {MEMORY_MAPPING_FILE} successfully")
        if os.path.exists(args.tf_idf_matrix_file):
            os.remove(args.tf_idf_matrix_file)
            print(
                f"Removed TF-IDF matrix file {args.tf_idf_matrix_file} successfully")
    ...
    if os.path.exists(TERM_DICTIONARY_FILE):
        ...
        document_reader = load_full_text_documents(
            scigraph_documents_path, num_documents_to_load=args.num_documents_to_load)
    ...
    if os.path.exists(args.tf_idf_matrix_file):
        ...
        tf_idf_matrix = compute_tf_idf_matrix(
            scigraph_documents_path, tf_idf_dictionary, num_documents_to_load=args.num_documents_to_load)
        sparse.save_npz(args.tf_idf_matrix_file, tf_idf_matrix)
        tf_idf_matrix = tf_idf_matrix.tolil()
        print(f"Saved TF-IDF matrix to {args.tf_idf_matrix_file}.")

    (out_edges, in_edges) = get_citation_graph(
        radius=1, remap_to_scirex_id=True)
    ...
    for graph_connectivity_type, adjacency_list in zip(adjacency_names, adjacency_lists):
        ...
        jsonfile = os.path.join(
            args.out_directory, f"{graph_connectivity_type}.json")
        per_term_tfidfs = compute_average_term_tfidfs_for_documents(
            adjacency_list, tf_idf_matrix, tf_idf_dictionary, doc_idx_mapping)
        json.dump(per_term_tfidfs, open(jsonfile, 'w'))
        print(
            f"Wrote JSON file with citation-graph-averaged TF-IDFs (over {graph_connectivity_type}) to {jsonfile}.")
