...
def fetch_metadata_download_commands():
    metadata_download_commands = open(
        metadata_download_script).read().split("\n")
    metadata_download_commands = [
        x.split() for x in metadata_download_commands if len(x.split()) != 0]
...
def fetch_s2orc_keys_from_scirex_ids(scirex_doc_ids, data_download_commands):
    ...
    if os.path.exists(scirex_s2orc_mapping_file):
        ...
        for i, s2orc_shard_command in enumerate(data_download_commands):
            ...
            output_path = f"{s2orc_shard_command[2]}"
...
def query_s2orc_document_in_scirex_dict(s2orc_doc, field_dicts):
    ...
    if s2orc_paper_id != None:
        scirex_id = field_dicts[0].get(s2orc_paper_id)
        if scirex_id != None:
            print(
                f">> SciREX document {scirex_id} matched on paper ID: {s2orc_paper_id}")
    ...
    if s2orc_doi != None:
        ...
        if scirex_id != None:
            print(
                f">> SciREX document {scirex_id} matched on DOI: {s2orc_doi}")
            for fdict in field_dicts:
                delete_keys_with_value(fdict, scirex_id)
            return scirex_id

    s2orc_arxiv_id = s2orc_doc.get("arxiv_id")
    if s2orc_arxiv_id != None:
        scirex_id = field_dicts[2].get(s2orc_arxiv_id)
        if scirex_id != None:
            print(
                f">> SciREX document {scirex_id} matched on arxiv ID: {s2orc_arxiv_id}")
            for fdict in field_dicts:
                delete_keys_with_value(fdict, scirex_id)
            return scirex_id
    ...
    if s2orc_title != None:
        ...
        if scirex_id != None:
            print(
                f">> SciREX document {scirex_id} matched on paper title: {s2orc_title}")
            for fdict in field_dicts:
                delete_keys_with_value(fdict, scirex_id)
            return scirex_id
...
def fetch_s2orc_meta_rows_from_scirex_ids(scirex_document_metadata, metadata_download_commands, overwrite_cache=False, num_shards_to_use=None):
    ...
    scirex_s2orc_metadata_file = os.path.join(
        caches_directory, "scirex_id_to_s2orc_metadata.pkl")
    if os.path.exists(scirex_s2orc_metadata_file) and not overwrite_cache:
        scirex_s2orc_metadata = pickle.load(
            open(scirex_s2orc_metadata_file, 'rb'))
    else:
        # If we don't have a cache file already, then manually match s2orc and scirex entries
        ...
        for i, s2orc_shard_command in enumerate(metadata_download_commands):
            ...
            shard_id = get_shard_id_from_path(
                output_path, data_type="metadata")
            ...
            for doc in s2orc_metadata:
                scirex_hit = query_s2orc_document_in_scirex_dict(
                    doc, field_dicts)
                if scirex_hit != None:
                    hits += 1
            ...
            if num_shards_to_use != None and i + 1 >= num_shards_to_use:
                break
        print(
            f"Found {all_hits} total matching documents in {time.perf_counter() - function_start} seconds")
        pickle.dump(scirex_s2orc_metadata, open(
            scirex_s2orc_metadata_file, 'wb'))
...
def create_citation_graph_from_seed_nodes(seed_node_ids, metadata_download_commands, graph_radius=2, overwrite_cache=False, num_shards_to_use=None):
    ...
    citation_graph_file = os.path.join(
        caches_directory, f"citation_graph_radius_{graph_radius}.pkl")
    if os.path.exists(citation_graph_file) and not overwrite_cache:
        citation_graph_adjacency_lists = pickle.load(
            open(citation_graph_file, 'rb'))
        print(
            f"Loading citation graph of radius {graph_radius} took {time.perf_counter() - function_start} seconds")
    else:
        out_edges = {}
        in_edges = {}
        for radius in range(1, graph_radius + 1):
            ...
            citation_graph_file = os.path.join(
                caches_directory, f"citation_graph_radius_{radius}.pkl")
            if os.path.exists(citation_graph_file) and not overwrite_cache:
                citation_graph_adjacency_lists = pickle.load(
                    open(citation_graph_file, 'rb'))
                (out_edges, in_edges) = citation_graph_adjacency_lists
                cited_nodes = [out_id for out_neighbors in out_edges.values()
                               for out_id in out_neighbors]
                citing_nodes = [in_id for in_neighbors in in_edges.values()
            ...
            for i, s2orc_shard_command in enumerate(metadata_download_commands):
                ...
                print(
                    f"Took {end - start} seconds to download shard {shard_id}\n")
                start = end
                # Load and match metadata rows
                shard = gzip.open(output_path, 'rt')
                s2orc_metadata = jsonlines.Reader(shard)
                ...
                print(
                    f"{nodes_added} new documents added from graph search in shard {shard_id}, from {hits} query nodes")
                print(
            ...
            pickle.dump(citation_graph_adjacency_lists,
                        open(citation_graph_file, 'wb'))
            print(
                f"Creating citation graph of radius {graph_radius} took {time.perf_counter() - function_start} seconds")
...
def construct_neighbor_text(seed_node_ids, metadata_download_commands, full_text_download_commands, out_directory=f"{caches_directory}/fulltexts", overwrite_cache=False, num_shards_to_use=None):
    function_start = time.perf_counter()
    scigraph_documents_path = os.path.join(
        out_directory, "scigraph_full_documents.jsonl")
    ...
    if os.path.exists(doc_to_index_path) and os.path.exists(doc_to_index_path):
        ...
        (out_edges, in_edges) = create_citation_graph_from_seed_nodes(seed_node_ids, metadata_download_commands,
                                                                      graph_radius=1, overwrite_cache=overwrite_cache, num_shards_to_use=num_shards_to_use)
        all_neighbors = [v for to_nodes in out_edges.values(
        ) for v in to_nodes] + [v for from_nodes in in_edges.values() for v in from_nodes]
        ...
        writer.close()
        print(
            f"Wrote {len(doc_idx_mapping)} jsonlines to {scigraph_documents_path}")
...
def get_scirex_s2_metadata():
    ...
    s2orc_hash_to_struct_mapping = fetch_s2orc_keys_from_scirex_ids(
        scirex_paper_ids, full_data_download_commands)
    scirex_s2_metadata = get_semantic_scholar_metadata(
        scirex_paper_ids, s2orc_hash_to_struct_mapping)
    scirex_s2orc_metadata = fetch_s2orc_meta_rows_from_scirex_ids(
        scirex_s2_metadata, metadata_download_commands)
    return scirex_s2orc_metadata
...
def get_citation_graph(radius, remap_to_scirex_id=False):
    ...
    (out_edges, in_edges) = create_citation_graph_from_seed_nodes(
        scirex_paper_ids, metadata_download_commands, graph_radius=radius)
...
def get_scirex_neighbor_texts():
    ...
    doc_idx_mapping, scigraph_documents_path = construct_neighbor_text(
        scirex_paper_ids, metadata_download_commands, full_data_download_commands, overwrite_cache=False)
    return doc_idx_mapping, scigraph_documents_path
...
def construct_graphvite_graph_format(citation_graph,
    ...
    print(
        f"Done generating {num_negative_edges_generated} negative edges: took {end - start} seconds")
    ...
    print(
        f"Creating {directedness} edge lists took {time.perf_counter() - function_start} seconds.")
...
def main():
    ...
    s2orc_hash_to_struct_mapping = fetch_s2orc_keys_from_scirex_ids(
        all_scirex_docids, full_data_download_commands)
    print("Done.\n\n")

    print("Get SciREX document metadata from Semantic Scholar API:")
    scirex_s2_metadata = get_semantic_scholar_metadata(
        all_scirex_docids, s2orc_hash_to_struct_mapping)
    print("Done.\n\n")
    ...
    scirex_paper_ids = [doc['paper_id']
                        for doc in scirex_s2orc_metadata.values()]
