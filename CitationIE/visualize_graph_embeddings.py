...
def visualize_tsne(tsne_data, name2rowidx, scirex_node_id_mapping, sampling_rate=0.01, initial_text_size=0.5, entity_type_to_label="Task"):
    scirex_idxs = np.array([name2rowidx[str(node_id)]
                           for node_id in scirex_node_id_mapping.keys()])
    non_scirex_idxs = [d for d in range(
        len(tsne_data)) if d not in scirex_idxs]
    sampled_idxes = np.random.choice(
        non_scirex_idxs, int(sampling_rate * len(tsne_data)))
    ...
    print(
        f"non_scirex_data_subsampled.shape: {non_scirex_data_subsampled.shape}")
    palette = [
        'purple',  # multimodal
        'blue',  # vision
        'green',  # games
    ...
    ax2 = sns.scatterplot(
        x=non_scirex_data_subsampled[:, 0], y=non_scirex_data_subsampled[:, 1], marker=".", s=0.6, palette=['black'])
    ax2 = sns.scatterplot(x=scirex_data[:, 0], y=scirex_data[:, 1],
                          marker="o", hue=doc_topics, s=2.0, palette=palette)
    ...
    print(
        f"task_abbreviations: {json.dumps(task_abbreviations, sort_keys=True, indent=4)}")
    ...
    for entity_name, node_id in entity_node_ids.items():
        for n in entity_node_ids[entity_name]:
            ...
            if not nearby_label_exists:
                text_coordinates_to_draw.append(
                    (entity_name, np.array([c1, c2]), initial_text_size))
            else:
    ...
    plt.title(
        f"Citation graph embeddings annotated with {entity_type_to_label} labels")
...
def nearest_neighbors(embeddings, name2idx, input_s2orc_id, s2orc_id_to_node_id_mapping, nearest_neighbors=5):
    ...
    print(
        f"Took {end-start} seconds to compute nearest neighbors over {len(embeddings)} embeddings")
...
def main():
    scirex_train = jsonlines.open(
        "SciREX/scirex_dataset/release_data/train.jsonl")
    scirex_test = jsonlines.open(
        "SciREX/scirex_dataset/release_data/test.jsonl")
    ...
    embeddings, name2id = unpack_embeddings("graphvite/graph.pkl")

    tsne_data = compute_tsne(embeddings, overwrite_cache=False)
    scirex_to_s2orc_mappings = get_scirex_to_s2orc_mappings()
    s2orc_id_to_node_id_mapping = json.load(
        open("ScigraphIE/node_id_mapping.json"))
    ...
    visualize_tsne(tsne_data, name2id, node_id_to_scirex_metadata,
                   entity_type_to_label="Task")
    visualize_tsne(tsne_data, name2id, node_id_to_scirex_metadata,
                   entity_type_to_label="Method")
    visualize_tsne(tsne_data, name2id, node_id_to_scirex_metadata,
                   entity_type_to_label="Metric")
    visualize_tsne(tsne_data, name2id, node_id_to_scirex_metadata,
                   entity_type_to_label="Material")

    input_s2orc_id = "630188"
    nearest_neighbors(embeddings, name2id, input_s2orc_id,
                      s2orc_id_to_node_id_mapping)
