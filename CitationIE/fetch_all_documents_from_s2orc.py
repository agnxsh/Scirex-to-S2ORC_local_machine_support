...
def download_s2orc_full_text(download_commands, candidate_set, data_directory, data_type, string_match_map=None, reference_match_map=None, num_shards_to_use=None):
    for i, s2orc_shard_command in enumerate(download_commands):
        ...
        for doc in s2orc_metadata:
            ...
            if paper_id in candidate_set:
                match = False
                if reference_match_map != None and paper_id in reference_match_map:
                    for dataset_canonical_name in reference_match_map[paper_id]:
                        shard_paper_to_datasets_map[paper_id].add(
                            ("reference", dataset_canonical_name))
                    match = True
                if string_match_map != None:
                        ...
                        if len(section_text) > 0:
                            for dataset_variant, dataset_canonical_name in string_match_map.items():
                                if dataset_variant in section_text:
                                    shard_paper_to_datasets_map[paper_id].add(
                                        ("mention", dataset_canonical_name))
                                    match = True
                                    break
        ...
        print(f"Wrote {hits} hits from shard {shard_id} to {shard_file_name}.")
        print(
            f"Processing {searched_documents} docs in shard took {end - start} seconds.")

        paper_to_datasets_map_file = os.path.join(
            data_directory, f"paper_to_datasets_map_shard_{shard_id}.json")
        print(f"Paper-to-Datasets Map is at {paper_to_datasets_map_file}")
        shard_paper_to_datasets_map = {
            k: [list(tup) for tup in v] for k, v in shard_paper_to_datasets_map.items()}
        json.dump(shard_paper_to_datasets_map, open(
            paper_to_datasets_map_file, 'w'))

        all_shard_index_file = os.path.join(
            data_directory, f"shard_index_shard_{shard_id}.json")
        print(f"Shard index file is at {all_shard_index_file}")
        json.dump(shard_index, open(all_shard_index_file, 'w'))
...
def main():
    ...
    citation_graph_edges = [os.path.join(caches_directory, f) for f in os.listdir(
        caches_directory) if f.startswith("citation_graph_radius_")]
    citation_graph_edges = [pickle.load(open(f, 'rb'))
                            for f in citation_graph_edges]
    ...
    print(
        f"Took {time.perf_counter() - start} seconds to load paper IDs in neighborhood.")
    ...
    reference_matches = json.load(open(
        "/projects/ogma2/users/vijayv/extra_storage/s2orc_caches/s2orc_papers_citing_datasets.json"))
    ...
    for dataset_meta in pwc_datasets:
        dataset_name_lookup_map[dataset_meta["name"].lower()
                                ] = dataset_meta["name"]
